...
- [time](time.md) — sibling "canonical primitive" pattern: clock reads and timestamp formats go through one crate. The same shape (one crate, one newtype, one contract) governs `phenotype-time` and `phenotype-secret`; consumers should not reach past either.
