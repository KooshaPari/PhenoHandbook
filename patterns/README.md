...
| Logging | [logging](logging.md) | Structured logging only — `phenotype_logging::init_tracing` (Rust) or `log/slog` with a JSON handler (Go). No `println!` / `eprintln!` / `log.Printf`. |
