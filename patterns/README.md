...
| Rust | [clippy-hygiene](clippy-hygiene.md) | `cargo clippy --workspace --all-targets -- -D warnings` must be clean; `[workspace.lints.clippy]` in root `Cargo.toml` + `lints.workspace = true` in every member crate. Wave 3 patched PhenoVCS, PhenoRuntime, PhenoMCP, PhenoAgent, PhenoPlugins, Eidolon. |
