...
│   └── ...
├── PhenoHandbook-wtrees/                               ← (this is where *this* branch's worktree would live)
...
- [shared-primitive-reuse](shared-primitive-reuse.md) — A primitive is promoted out of a consumer into `phenoShared/crates/` *only* when it's the second use. Workspace organization is what makes the second use visible: the consumer points at the new `phenoShared` crate by `path = "../phenoShared/crates/<name>"`, and the workspace takes care of the rest.
...
- [phenotype-registry/ECOSYSTEM_MAP.md](https://github.com/KooshaPari/phenotype-registry) — The canonical list of which repo lives where, in spine order. Update this page and the registry's `ECOSYSTEM_MAP.md` together; the registry is authoritative for "what exists," this page is authoritative for "where it sits on disk."
